...
output$retire_line <- plotly::renderPlotly({
  ...
  plotly::ggplotly(p, tooltip = c("x", "y")) %>% 
    ...
    plotly::layout(
      ...
      modebar = list(x = 1, xanchor = "right", y = 1, yanchor = "top"),
        orientation = "v",
        bgcolor = "rgba(255,255,255,0.7)",
        x = 1.05,   # push to the right (try 1.02–1.10)
        xanchor = "left",
        y = 1,      # top
        yanchor = "top"
      )
      margin = list(t = 80)  # add top space so title has room
  })

output$retire_line <- renderPlotly({
  d <- metric_data()
...
# Filter for slider
retirement_filtered <- reactive({
  retirement_data %>%
    filter(Year >= input$retire_year[1],
           Year <= input$retire_year[2])
})

# Interactive plot
output$retire_plot <- renderPlotly({
  ...
  p <- ggplot(
    ...
    aes(
      ...
      text = paste(
        "Year:", Year,
        "<br>Employment Rate:",
        scales::percent(Employment_Rate, accuracy = 0.1)
      )
    )
  ) +
  ...
  if (input$retire_points) {
    p <- p + geom_point(color = "#264653", size = 2)
  }
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput(
        "retire_year",
        "Select Year Range:",
        min = min(retirement_data$Year),
        max = max(retirement_data$Year),
        value = c(min(retirement_data$Year), max(retirement_data$Year)),
        step = 1,
        sep = ""
      ),
      checkboxInput("retire_points", "Show Points", value = TRUE),
      helpText("This chart looks at employment rate for workers age 55+ over time.")
    ),
    mainPanel(
      plotlyOutput("retire_plot")
    )
  )
)
=======
...
theme_minimal()
```
>>>>>>> cc7c59c64d05cae9a1ec81a91ace2587cd624343
