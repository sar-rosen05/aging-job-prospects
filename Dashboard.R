...
."),
             ...
             strong("Methods:"),
             p("The unemployment rate for each racial group was calculated using data from the Table 8 dataset. For each year, the labor force was defined as the total number of employed workers (full-time and part-time) plus unemployed workers. The unemployment rate was then calculated by dividing the number of unemployed individuals by the total labor force. After calculating the rate for each race and year, the data was grouped and averaged to produce a single unemployment rate value for each racial group annually. The results are displayed using an interactive line chart built with ggplot2 and plotly, which allows users to select specific racial groups and adjust the year range to explore trends over time."),
             strong("Limitations:"),
             p("This visualization has several limitations. First, the data aggregates individuals into broad racial categories, which may hide differences within each group. Second, the unemployment rate only reflects whether individuals are employed or unemployed and does not capture other important factors such as job quality, wages, hours worked, or underemployment. Additionally, the dataset does not account for other social or economic factors that may influence unemployment patterns, such as education level, geographic location, or industry. Finally, the analysis uses yearly averages, which may smooth out short-term economic changes that occurred within a given year."),
